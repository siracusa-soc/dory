...
#define NEUREKA_CG_ENABLE()  *(volatile int*) (CLUSTER_CTRL_BASE_ADDR + CLUSTER_CTRL_HWPE_OFFS) |=  CLUSTER_CTRL_HWPE_CG_EN_MASK
#define NEUREKA_CG_DISABLE() *(volatile int*) (CLUSTER_CTRL_BASE_ADDR + CLUSTER_CTRL_HWPE_OFFS) &= ~CLUSTER_CTRL_HWPE_CG_EN_MASK
...
#define NEUREKA_READ_IO_REG(offset) NEUREKA_READ(NEUREKA_REGISTER_OFFSET + (offset))

#define NEUREKA_BARRIER_NOSTATUS()      eu_evt_maskWaitAndClr (1 << NEUREKA_EVT0)
#define NEUREKA_BARRIER()               do { eu_evt_maskWaitAndClr (1 << NEUREKA_EVT0); } while((*(int volatile *)(NEUREKA_BASE_ADDR + NEUREKA_STATUS)) != 0)
#define NEUREKA_BUSYWAIT()              do {                                         } while((*(int volatile *)(NEUREKA_BASE_ADDR + NEUREKA_STATUS)) != 0)
#define NEUREKA_BARRIER_ACQUIRE(job_id) job_id = NEUREKA_READ(NEUREKA_ACQUIRE); \
                                     while(job_id < 0) { eu_evt_maskWaitAndClr (1 << NEUREKA_EVT0); job_id = NEUREKA_READ(NEUREKA_ACQUIRE); };
#define NEUREKA_NOBARRIER_ACQUIRE(job_id) job_id = NEUREKA_READ(NEUREKA_ACQUIRE); \
                                       while(job_id < 0) { job_id = NEUREKA_READ(NEUREKA_ACQUIRE); };
...
int nnx_pad_input(nnx_cfg_t *cfg, uint32_t top, uint32_t right,
                   uint32_t bottom, uint32_t left, uint16_t value);
...
nnx_error_code nnx_conv_1x1(nnx_cfg_t *cfg, nnx_weights_t weights, nnx_feature_t input, nnx_feature_t output);
nnx_error_code nnx_conv_1x1_update_dims(nnx_cfg_t *cfg, int h_out, int w_out, int k_out, int k_in);
nnx_error_code nnx_conv_3x3(nnx_cfg_t *cfg, nnx_weights_t weights, nnx_feature_t input, nnx_feature_t output);
nnx_error_code nnx_conv_3x3_update_dims(nnx_cfg_t *cfg, int h_out, int w_out, int k_out, int k_in);
nnx_error_code nnx_conv_3x3_dw(nnx_cfg_t *cfg, nnx_weights_t weights, nnx_feature_t input, nnx_feature_t output);
nnx_error_code nnx_conv_3x3_dw_update_dims(nnx_cfg_t *cfg, int h_out, int w_out, int k_out, int k_in);
